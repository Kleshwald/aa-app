...
│   ├── profile/              ✅ профиль на 4 вкладки (Данные/реквизиты · Документы · Условия/КВ · Команда)
│   ├── team/                 ✅ панель «Команда» (сотрудники+субагенты) — вкладка в Профиле, не роут
...
- **Sidebar — 8 пунктов:** Мои клиенты · Пролонгация · ОСАГО · Здоровье · Ипотека · Мои финансы · Обучение · Сообщения. **«Команда»** (сотрудники + субагенты для ИП со штатом) — НЕ в сайдбаре, а **4-й вкладкой в Профиле** (перенёс владелец 2026-06-25; кандидат на условную видимость — одиночке-ФЛ не нужна).
...
- **Страница договора `/clients/:id` — процессы и кросс-сел показывать ЗАМЕТНО** (не прятать!). «Процессы» (внести изменения · расторжение · урегулировать убыток) — **ключевое отличие от конкурентов** (у них — по почте; у нас автоматизировано), специально на виду каждый раз. **Кросс-продукты** — главная маржа: на странице договора можно докупить **до 3-го допа** (при оплате ОСАГО — только 1). Цены допов из `ADD_ON_PRESETS` (osago): Юрист от 500 ₽, НС при ДТП от 800 ₽, **МиниКАСКО 2450 ₽**. Горячий сценарий (`?issued=1`) — плашка успеха + «отдать полис клиенту», но процессы/кросс-сел тоже доступны. Преемственность с 1С важна.
...
- В проекте больше одного участника: **второй продакт** работает со своим AI (Codex/Cursor) **ветками + PR**, не в main. Его онбординг — [docs/ONBOARDING.md](./docs/ONBOARDING.md); контекст для чужих AI-инструментов — **[AGENTS.md](./AGENTS.md)** (курируемый свод правил и залоченных решений — держать синхронно с CLAUDE.md, /hub и памятью).
- **Пуш в `main` = автодеплой на прод** (GitHub Pages, `.github/workflows/deploy.yml`). Релиз — только по явной команде владельца; практика — build+lint из изолированного `git worktree` на чистом HEAD, если рабочее дерево держит чужой WIP.
